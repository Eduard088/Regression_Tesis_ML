# Regresion_lineal_local guide installation
...
activate regresion_lineal_local
```
...
To move beyond notebook prototyping, all reusable code should go into the `regresion_congreso_local/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `regresion_congreso_local` folder and use the modules inside your notebooks :
...
```python
from regresion_congreso_local.utils.paths import data_dir
data_dir()
```
